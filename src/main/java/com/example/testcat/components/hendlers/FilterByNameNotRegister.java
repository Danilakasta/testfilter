...
@Component
public class FilterByNameNotRegister implements AnimalFilterDef<AnimalAbstract,AnimalFilter> {
    @Override
    ...
    public Predicate<AnimalAbstract> condition(AnimalFilter filter) {
        return animalAbstract -> animalAbstract.getName().toUpperCase().equals(filter.getName().toUpperCase());
    }
